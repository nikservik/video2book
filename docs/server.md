...
+ `POST /api/projects/{project}/lessons` создаёт урок по загруженному аудиофайлу; если `pipeline_version_id` не передан, используется `projects.default_pipeline_version_id`, а необязательный `source_url` сохраняет ссылку на исходник урока.
...
+ При первом открытии страницы `Проекты`: если видима ровно одна папка, она раскрывается автоматически; если видимых папок больше одной, все стартуют закрытыми
+ Если в URL передан query-параметр `f` с id видимой папки, при загрузке страницы раскрывается эта папка
...
+ Для папок добавлены поля `hidden` (bool, default `false`) и `visible_for` (json-массив id пользователей)
+ В модалках создания/редактирования папки есть чекбокс `Скрыть папку`; при включении появляется список `Кому папка видна` (сортировка пользователей: `access_level DESC`, затем `name`)
+ Для `superadmin` и текущего редактора папки чекбоксы видимости принудительно включены и недоступны для снятия
...
+ Скрытые папки и проекты внутри них фильтруются по `visible_for` (правило применяется в `Проекты` и `Свежие проекты` на главной)
+ На главной странице под блоком `Свежие проекты` добавлен статический блок `Последние обновления`: заголовок расположен внутри фона блока, ниже выводится дата и ручной список свежих изменений продукта
...
+ По умолчанию проект скачивается одним файлом: результаты всех уроков объединяются в один markdown-документ, перед каждым блоком добавляется заголовок `# {Урок}`, а все внутренние заголовки результата поднимаются на уровень глубже (`#` → `##`)
...
+ В модале `Добавить урок` при изменении YouTube-ссылки (`wire:model.live`) выполняется проверка дублей по `lessons.settings.url` без query string; при совпадении под полем выводится предупреждение с ссылкой на существующий урок в формате `Проект - Урок`
...
+ Если у урока ровно один прогон, весь блок урока становится кликабельным и открывает этот прогон; клики по кнопкам/ссылкам внутри блока не перехватываются
+ В карточке урока кнопка-оверлей удаления прогона доступна только для `admin/superadmin`; для `access_level=0` скрыта
+ В breadcrumbs страниц `Проект` и `Прогон` ссылка `Проекты` передаёт query-параметр `f` с id папки текущего проекта
...
+ Имя скачиваемого файла для результата шага и ZIP-экспорта проекта сохраняет кириллицу и исходные названия урока/шага/проекта; из имени убираются только недопустимые файловые символы, без `slug`
...
+ Для результата шага добавлен режим редактирования через `Trix`: в режиме просмотра показывается кнопка `Править`, в режиме редактирования — кнопки `Отмена` и `Сохранить`
+ В редакторе `Trix` оставлены только `h1/h2/h3`, `bold`, `italic`, списки (`ul/ol`) и вложенность списков
+ Промежуточные правки в редакторе не сохраняются в БД; запись выполняется только по кнопке `Сохранить`
+ На сохранении результат проходит цепочку `HTML -> mews/purifier -> StepResultHtmlToMarkdownConverter` (кастомная DOM-конвертация разрешённых тегов), после чего записывается в `pipeline_run_steps.result`
+ В `pipeline_run_steps` добавлено поле `original`: при первом сохранении туда копируется исходный `result`, последующие сохранения меняют только `result`
+ Если у выбранного шага заполнено `original`, рядом с `Править` показывается кнопка восстановления; подтверждение в модале `Отмена / Восстановить` копирует `original` обратно в `result`
+ Для ручного редактирования в активность пишется описание `изменил текст в шаге ...`, для восстановления исходника — `восстановил текст в шаге ...`
+ После сохранения/восстановления результата `trix-editor` синхронизируется сразу в Livewire без ручной перезагрузки страницы
...
+ Модал редактирования: поле `Ссылка на MCP`, invite-ссылка, копирование обеих ссылок, кнопка `Новый токен`
...
+ Для типа `проект` при `updated`-событиях без `name` в `properties` название берётся из текущей записи проекта (с fallback на `Проект #ID`).
...
+ Для ручного сохранения текста шага прогона добавляется отдельная `updated`-запись по `PipelineRun` с описанием вида `{пользователь} изменил текст в шаге {N} в уроке «{урок}» проекта «{проект}».
