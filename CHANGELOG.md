...
- Для карточек задач и состояния `Очередь сейчас пуста.` установлен фон в стиле блока `Последние измененные проекты` (`bg-white`/`dark:bg-gray-800`) с сохранением карточной структуры.
