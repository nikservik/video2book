...
- В заголовке страницы прогона добавлены кнопки управления `Start / Pause / Stop`: `Start` показывается при наличии шагов `paused`, `Pause` и `Stop` — при наличии шагов `pending`; блок кнопок обновляется polling-ом `1s`.
