...
- Уровни доступа:
  ...
  - `1` — админ: дополнительно `Пайплайны`, `Пользователи`, `Активность`.
...
- Верхнее меню содержит разделы `Главная`, `Проекты`, `Пайплайны`, `Пользователи`, `Активность`; видимость пунктов зависит от `access_level`, а активный пункт определяется по имени роута (`routeIs`).
- В layout между меню и контентом добавлены breadcrumbs (адаптация Tailwind UI): на главной breadcrumbs скрыты, на остальных страницах (`Проекты`, `Проект`, `Урок`, `Пайплайны`, `Шаг пайплайна`) путь формируется через параметры `breadcrumbs` в full-page Livewire-компонентах; на мобильных включён перенос в несколько строк с левым сдвигом для 2+ строк, при этом связка `separator-icon + label` не разрывается.
...
- Страница `Пайплайны` отображает все пайплайны в grid на 3 колонки, для каждого показывает название, номер текущей версии и описание текущей версии; в заголовке есть кнопка `Добавить пайплайн`, открывающая модал создания с динамическим списком шагов.
...
- Страница `Активность` доступна для `admin/superadmin` и показывает историю `Activity Log` в формате строк (`дата и время`, `пользователь`, `действие`, `тип объекта`, `название объекта`) с пагинацией по 20 записей; для `прогона` название формируется как `Урок — Пайплайн • vN`, а для `урока` дописывается проект: `Урок в проекте «Проект»`.
...
- Карточки в списке пайплайнов кликабельны и ведут на страницу просмотра пайплайна (`pipelines.show`).
- На странице просмотра пайплайна отображаются шаги выбранной версии (по умолчанию — текущей версии пайплайна), а в правом блоке доступны кнопки `Редактировать версию`, `Сделать текущей версией` (disabled для текущей) и `Архивировать версию`/`Вернуть из архива` (в зависимости от статуса версии), а также переключение между версиями.
- Новая версия пайплайна, созданная из модала `Добавить пайплайн`, по умолчанию получает статус `archived`, потому что её шаги стартуют в `draft`; возврат из архива доступен только когда в выбранной версии нет шагов со статусом `draft`.
...
- Модал создания проекта содержит поля: `Название проекта` (обязательное), `Referer`, `Версия пайплайна по умолчанию`, `Список уроков`; выбор версии пайплайна вынесен в общий Blade-компонент `x-pipeline-version-select`.
...
- После нормализации аудио через `ffprobe` в `settings` урока сохраняется `audio_duration_seconds`; на карточке урока рядом с иконкой статуса загрузки выводится длительность в формате `HH:MM`.
...
- Если выбранный шаг завершился с ошибкой (`failed`) и `result` пустой, в блоке результата вместо заглушки выводится текст ошибки красным (в режимах `Превью` и `Исходник`).
...
- В меню действий проекта доступны кнопки `Добавить урок`, `Добавить урок из аудио`, `Добавить список уроков`, `Редактировать проект`, а также экспорт результатов в `PDF`, `MD` и `DOCX` (через общий modal выбора версии пайплайна и шага с последующей загрузкой ZIP-архива).
