...
- `POST /api/projects/{project}/lessons` — добавление урока через multipart-загрузку аудиофайла (`name`, `file`, опционально `pipeline_version_id` и `source_url`).
...
- Главная страница реализована как full-page Livewire-компонент (`app/Livewire/HomePage.php`) c данными из сервисного query-слоя (`app/Services/Project/RecentProjectsQuery.php`): левая колонка `2/3` показывает блок `Свежие проекты` — 5 последних изменённых проектов в табличном виде (как на странице `Проекты`), где первая колонка — название папки проекта, а колонки `Уроков` и `Длительность` стакаются на mobile (до `md`); под ним расположен статический блок `Последние обновления` с ручным HTML-списком свежих продуктовых изменений.
...
- На страницах `Проект` и `Прогон` breadcrumb-ссылка `Проекты` ведёт на `projects.index` с query-параметром `f={folder_id}` текущего проекта, чтобы при возврате сохранялось раскрытие нужной папки.
...
- При первом открытии страницы `Проекты` папка разворачивается автоматически только если она единственная видимая; если видимых папок больше одной, все стартуют закрытыми.
- Если в URL страницы `Проекты` передан query-параметр `f` с id доступной папки, при загрузке раскрывается именно эта папка (в том числе при возврате кнопкой «Назад»).
...
- Страница `Пользователи` доступна для `admin/superadmin`: список карточек, добавление/редактирование пользователя, удаление через confirm-модал, генерация нового invite-токена, поле `Ссылка на MCP` с копированием и копирование invite-ссылки.
- Страница `Активность` доступна для `admin/superadmin` и показывает историю `Activity Log` в формате строк (`дата и время`, `пользователь`, `действие`, `тип объекта`, `название объекта`) с пагинацией по 20 записей; для `прогона` название формируется как `Урок — Шаблон • vN`, для `урока` дописывается проект: `Урок в проекте «Проект»`, а для `проекта` при `updated`-событии без `name` в `properties` имя берётся из текущей записи проекта (иначе fallback `Проект #ID`). В лог попадают только действия с автором-пользователем; системные действия без `causer` не записываются.
...
- Если у урока ровно один прогон, весь блок урока на странице проекта получает hover-подсветку и ведёт в этот прогон по клику (кнопки и ссылки внутри блока остаются интерактивными и работают отдельно).
...
- В блоке результата шага добавлен режим редактирования через `Trix`: над контентом справа доступны кнопки `Править` (в режиме просмотра) и `Отмена/Сохранить` (в режиме редактирования).
- Если у выбранного шага уже есть `pipeline_run_steps.original`, рядом с `Править` показывается серая кнопка восстановления исходного текста; по нажатию открывается confirm-модал `Отмена / Восстановить`.
- Подтверждение восстановления копирует `pipeline_run_steps.original` обратно в `pipeline_run_steps.result` для текущего шага.
- После сохранения или восстановления результата блок `Trix` синхронизируется сразу в текущем Livewire-ответе, без ручного обновления страницы.
- В `Trix` для этого сценария оставлено ограниченное форматирование: `h1-h3`, `bold`, `italic`, нумерованные/маркированные списки и вложенность списков.
- Промежуточные изменения в редакторе не сохраняются в БД; запись происходит только по кнопке `Сохранить`.
- При сохранении результат проходит цепочку `HTML -> mews/purifier -> StepResultHtmlToMarkdownConverter` (кастомная DOM-конвертация только разрешённых тегов `h1-h3/strong/em/ul/ol/li/div/p/br`) и записывается обратно в `pipeline_run_steps.result`.
- В `pipeline_run_steps` добавлено поле `original`: при первом ручном сохранении туда копируется исходный markdown из `result`, последующие правки обновляют только `result`.
- Каждое сохранение правки результата шага пишет запись в `activity_log` с текстом вида `{пользователь} изменил текст в шаге {N} в уроке «{урок}» проекта «{проект}».
- Восстановление исходного текста шага пишет отдельную запись в `activity_log` с текстом вида `{пользователь} восстановил текст в шаге {N} в уроке «{урок}» проекта «{проект}».
...
- Имя скачиваемого файла на странице прогона и у ZIP-экспорта проекта сохраняет кириллицу и исходные названия урока, шага и проекта; из имени мягко удаляются только недопустимые для файловой системы символы, без `slug`-трансформации.
...
- В меню действий проекта доступны кнопки `Добавить урок`, `Добавить урок из аудио`, `Добавить список уроков`, `Редактировать проект`, `Пересчитать длительность` (запуск action пересчёта длительностей проекта), а также экспорт результатов в `PDF`, `MD` и `DOCX` (через общий modal выбора версии пайплайна и шага).
...
- В modal экспорта проекта по умолчанию выбран режим `Одним файлом`: результаты всех подходящих уроков объединяются в один markdown-документ с заголовками уроков первого уровня, а внутренние заголовки каждого результата сдвигаются на один уровень глубже.
- В том же modal можно переключиться на ZIP-режимы `Урок.{ext}` и `Урок - шаг.{ext}`; имя скачиваемого файла или ZIP формируется только из названия проекта.
...
- В модале `Добавить урок` при изменении YouTube-ссылки показывается предупреждение о дубле, если урок с таким URL (сравнение без query string) уже существует: `Урок с таким видео уже есть: {Проект} - {Урок}` (с кликабельной ссылкой на проект).
...
- Тесты принудительно запускаются только на `sqlite` c базой `:memory:` (см. `phpunit.xml`), даже если в shell экспортированы `DB_*` переменные для локальной `pgsql/mysql`.
