...
- Страница `Пайплайны` отображает все пайплайны в grid на 3 колонки, для каждого показывает название, номер текущей версии и описание текущей версии; в заголовке есть кнопка `Добавить пайплайн`, ведущая на отдельную страницу создания.
- Карточки в списке пайплайнов кликабельны и ведут на страницу просмотра пайплайна (`pipelines.show`).
- На странице просмотра пайплайна отображаются шаги выбранной версии (по умолчанию — текущей версии пайплайна), а в правом блоке доступны кнопки `Редактировать версию` и `Архивировать версию`/`Вернуть из архива` (в зависимости от статуса версии), а также переключение между версиями.
