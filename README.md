...
- В карточке урока на странице проекта выводится список прогонов (`PipelineRun`): для каждого прогона отображаются название и версия пайплайна (`Название • vN`) и статусный badge в стиле Tailwind UI (`Готово`, `В очереди`, `Обработка`).
